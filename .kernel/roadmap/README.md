...
| [feat-sow-branch-config](feat-sow-branch-config/feat-sow-branch-config.md) | | ⏳ todo | high | | | Bootstrap records the SOW lead and each user's personal branch so Claude always knows where to commit and `/2ndbrain` warns when setup is incomplete. |
