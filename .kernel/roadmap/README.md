...
| [challenge-sow-task-board-visibility](archived/challenge-sow-task-board-visibility/challenge-sow-task-board-visibility.md) | | ✅ done | medium | | | Root-cause finding that the SOW task table's brittleness (column order, table dialects, one shared file) is what makes cross-source task visibility hard — fixed by `feat-sow-tasks-as-files`. |
| [feat-sow-tasks-as-files](archived/feat-sow-tasks-as-files/feat-sow-tasks-as-files.md) | | ✅ done | medium | | | Replaced the shared `sows/<sow>/<sow>-tasks.md` table with one frontmatter file per task under `sows/<sow>/tasks/`, closed tasks moving to `tasks/done/` — robust parsing, per-task git history, room for a real resolution log. |
