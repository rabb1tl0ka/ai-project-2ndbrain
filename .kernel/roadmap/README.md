...
| [feat-sow-branch-config](feat-sow-branch-config/feat-sow-branch-config.md) | | ✅ done | high | | | `/onboard` asks for the SOW lead, writes `sow_lead` to `sow.config.yaml`, and creates the long-lived `sow{N}` branch on GitHub with a suggestion to protect it. |
